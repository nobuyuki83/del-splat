...
pub fn read_xyzrgb<Path: AsRef<std::path::Path>>(
    path: Path,
    ...
    let mut pnt2xyz: Vec<f32> = vec![];
    let mut pnt2rgb: Vec<f32> = vec![];
    for i_elem in 0..num_elem {
        ...
        pnt2xyz.extend_from_slice(&[x as f32, y as f32, z as f32]);
        pnt2rgb.extend_from_slice(&[(r as f32) / 255.0, (g as f32) / 255.0, (b as f32) / 255.0]);
    }
    Ok((pnt2xyz, pnt2rgb))
}
...
pub fn read_3d_gauss_splat<Path: AsRef<std::path::Path>>(
    ...
    let mut pnt2xyz: Vec<f32> = vec![];
    let mut pnt2rgb: Vec<f32> = vec![];
    let mut pnt2sh: Vec<f32> = vec![];
    let mut pnt2op: Vec<f32> = vec![];
    let mut pnt2scale: Vec<f32> = vec![];
    let mut pnt2quat: Vec<f32> = vec![];
    for i_elem in 0..num_elem {
        ...
        pnt2xyz.extend_from_slice(&xyz);
        pnt2rgb.extend_from_slice(&rgb);
        pnt2sh.extend_from_slice(&sh);
        pnt2op.push(op);
        pnt2scale.extend_from_slice(&scale);
        pnt2quat.extend_from_slice(&quaternion);
        // pnt2gs3.push(Splat::new(xyz, rgb, sh, op, scale, quaternion));
    }
    Ok((pnt2xyz, pnt2rgb, pnt2sh, pnt2op, pnt2scale, pnt2quat))
}
